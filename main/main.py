...
# --- sound / image stubs (replace with pygame.mixer and images as needed) ---
# --- Asset filenames (your provided tracks) ---
SOUND_FILES = {
    "ghost": "Ghost Speak Get Out - QuickSounds.com.mp3",
    "phasmophobia": "Phasmophobia - QuickSounds.com.mp3",
    "blood_splat": "BloodSplat - QuickSounds.com.mp3",
    "window_break": "window-breaking-105533.mp3",
    "rip_blood_breach": "RipBlood - QuickSounds.com.mp3",
    "ghost_breath": "Ghostly Breath - QuickSounds.com.mp3",
    "whisper_flashes": "Whisper Ghost - QuickSounds.com.mp3",
    "syringe_inject": "SyringeInject - QuickSounds.com.mp3",
    "voice_isee_you": "VoiceISeey - QuickSounds.com.mp3",
}

# --- Load sounds safely into pygame.mixer.Sound objects ---
sounds = {}
for key, fname in SOUND_FILES.items():
    if os.path.exists(fname):
        try:
            # Use pygame.mixer.Sound for short effects; for longer MP3s consider pygame.mixer.music
            sounds[key] = pygame.mixer.Sound(fname)
        except Exception as e:
            print(f"[sound load error] {fname}: {e}")
            sounds[key] = None
    else:
        print(f"[sound missing] {fname} not found")
        sounds[key] = None

# --- Sound play helpers (no-op if asset missing) ---
def play_sound(key, volume=1.0):
    ...
    if s:
        ...
        # fallback print so game still communicates sound events
        print(f"[sound missing] {key}")

def play_ghost_sound():
    play_sound("ghost")

def play_phasmophobia():
    play_sound("phasmophobia")

def play_BloodSplat():
    play_sound("blood_splat")

def play_windowBreaking():
    play_sound("window_break")

def play_RipBloodBreach():
    play_sound("rip_blood_breach")

def play_GhostlyBreath():
    play_sound("ghost_breath")

def play_WhisperGhostFlashes():
    play_sound("whisper_flashes")

def play_SyringeInjectBloodPump():
    play_sound("syringe_inject")

def play_VoiceISeeYou():
    play_sound("voice_isee_you")
